...
function resize(width,height){
    ...
    if(!$.isEmptyObject($('#tabs').get(0))) {
    	$("#tabs").tabs({ 
	    		width:width+3, 
	    		height:height 
    		}); 
    }
    if(!$.isEmptyObject($('.tabsgrid').get(0))) {
        $('.tabsgrid').datagrid({fit:true});
    }
}
/**
 * 引用外部JS
    ...
    $.extend($.fn.validatebox.defaults.rules, {
        requiredSelect: {
          validator: function(value, param){
                return param ? ($.trim(value) != '' && $.trim(value) != '==select==') : false;
            },
        ...
        number: {
            ...
            message: 'The entry can only enter numbers'
        },
        znumber: {
            validator: function(value, param){
                return !/[1-9]\D/g.test(value);
            },
            message: 'Only positive integers can be entered for this input'
        },
        QQ: {
            validator: function(value, param){
            	var rec;  
        ...
        numberval: {
            ...
            message: 'Please enter a number in the correct format,Enter up to 3 decimal places'
        },
        numberval1: {
            validator: function(value, param){
                if(/^[0-5]+(\.{1,1})?$/.test(value)){
                    if(value>1000000){
                        return false;
                    }
                    return true;
            ...
            message: 'Please enter a number in the correct format,Enter up to 1 decimal places'
        },
        numberv: {
            validator: function(value, param){
                if(value.length>9){
            ...
            message: 'Please enter a valid number up to 9 characters long'
        },
        numberlength: {
            validator: function(value, param){
                if(isNaN(value)){
...
//                alert(numberarry.length);
//                if(value.indexOf('.')>0){
//                    //alert('有小数点');
//                }
                return true;
            },
            message: 'Please enter an effective percentage of no more than 100%'
        },
        double: {
            validator: function(value, param){
                if(isNaN(value)){
        ...
        money: {
            ...
            message: 'Please enter a valid monetary figure of no more than 1000000'
        },
        username: {
            validator: function(value, param){
        ...
        warnkey: {
            ...
            message: 'Please enter a canonical field'
        },
        password: {
            validator: function(value, param){
                return /^[a-zA-Z0-9_-]{6,18}$/.test(value);
            },
            message: '请输入6-18位有效密码（数字或字母）！'
        },
        verify_password: {
            validator: function(value, param){
    ...
    $.extend($.fn.validatebox.defaults.rules, {
        integer: {
            ...
            message: 'Please enter a positive integer from 0 to 99999999'
        }
