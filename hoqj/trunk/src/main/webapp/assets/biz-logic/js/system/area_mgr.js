$(function(){
    function addHoverDom(treeId, treeNode) {
        ...
        if (btn) btn.bind("click", function(){
            ...
            var node = {isParent:false, parentId:treeNode.id, parentName:treeNode.name,id:''};
    ...
    var setting = {
        ...
        callback : {
            ...
            beforeRemove:function(treeId, treeNode) {
                ...
                $.messager.confirm('提醒','确定要删除该'+(treeNode.isParent ? '及其子' : '')+'区域吗？',function(r) {
                    if(r){
                        $._ajaxPost('area/del.html',{id:treeNode.id},function(r){
                            if(r.r) {
    ...
    $('#menuEditForm').submit(function(){
        ...
        if($.trim(node.menuId) == '') {
            $._ajaxPost('area/add.html', node, function(r){
                if(r.r) {
                    var parentNode = zTree.getNodeByParam('id', node.parentId, null);
                    node.id = r.id;
                    node.showName = $.trim(node.remark) == '' ? node.name : node.name + '[' + node.remark + ']';
            ...
            $._ajaxPost('area/upd.html', node, function(r){
                if(r.r) {
